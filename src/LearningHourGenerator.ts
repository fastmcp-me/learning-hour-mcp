...
- The progression should tell a story of incremental improvement`;
  ...
  async generateSessionWithCodeExamples(topic: string, style: string = 'slide', language: string = 'java'): Promise<SessionContent> {
    ...
    const initialCodeSection = {
      title: 'Starting Code - ' + codeExample.context,
      type: 'code_examples',
      language: codeExample.language,
      content: codeExample.problemStatement + '\\n\\n' + codeExample.learningHourConnection,
      beforeCode: codeExample.refactoringSteps[0].code,
      afterCode: ''
    };

    const refactoringStepSections = codeExample.refactoringSteps.map((step, index) => ({
      title: `Refactoring Step ${step.stepNumber}: ${step.description}`,
      type: 'code_examples',
      language: codeExample.language,
      content: `Code Smells: ${step.codeSmells.join(', ')}\\n\\nImprovements: ${step.improvements.join(', ')}\\n\\nFacilitation: ${step.facilitationTip}`,
      beforeCode: step.code,
      afterCode: step.testCode || ''
    }));

    // Add facilitation notes section
    const facilitationSection = {
      title: 'Facilitation Guide',
      type: 'sticky_notes',
      color: 'light_blue',
      items: [
        'Time: ' + codeExample.facilitationNotes.timeAllocation,
        ...codeExample.facilitationNotes.pairProgrammingTips,
        ...codeExample.facilitationNotes.discussionPoints.map(point => 'Discuss: ' + point)
      ]
    };
    ...
    const mistakesSection = {
      ...
      type: 'sticky_notes',
      color: 'light_red',
      items: codeExample.facilitationNotes.commonMistakes
    };
    ...
    const exercisesSection = {
      ...
      type: 'sticky_notes',
      color: 'light_green',
      items: codeExample.additionalExercises
    };

    // Insert code-related sections after the Concrete activity section
    const concreteIndex = sessionContent.miroContent.sections.findIndex(s => s.title.includes('CONCRETE'));
    if (concreteIndex !== -1) {
      sessionContent.miroContent.sections.splice(
        concreteIndex + 1,
        0,
        initialCodeSection,
        ...refactoringStepSections,
        facilitationSection,
        mistakesSection,
        exercisesSection
      );
    } else {
      sessionContent.miroContent.sections.push(
        initialCodeSection,
        ...refactoringStepSections,
        facilitationSection,
        mistakesSection,
